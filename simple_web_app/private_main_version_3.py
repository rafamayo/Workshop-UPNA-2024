...
#    'api_base': 'https://fhir.server/baseDstu3/'  # Default FHIR server URL
    'api_base': 'http://localhost:8080/fhir/'  # Default FHIR server URL
}
...
def new_patient():
    ...
    if request.method == 'POST':
        ...
        # Check if a practitioner has been selected
        if practitioner_id:  # If practitioner_id is not None and not an empty string
            ...
            message = "A practitioner must be selected to create a new patient."
        
        # Return with the practitioners list and the message
        return render_template('new_patient.html', practitioners=practitioner_list, message=message)
    
    # For a GET request or if there's no form submission, just show the form
    return render_template('new_patient.html', practitioners=practitioner_list, message=message)
